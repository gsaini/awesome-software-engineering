...
| [Infrastructure as Code (Terraform)](infrastructure-as-code-terraform.md) | 🟡 | Provisioning infra as version-controlled config — declarative desired-state, the plan/apply workflow, state (remote/locking/secrets), drift, modules & environments, GitOps & policy-as-code, and the landscape. |
| [Containers & Orchestration](containers-orchestration.md) | 🟡 | Containers vs. VMs, how a container works (namespaces/cgroups/overlayfs), images & layers, Docker/OCI, why orchestration, Kubernetes' reconcile loop & core objects, the ecosystem, and when *not* to use K8s. |
| [Feature Flags & Progressive Delivery](feature-flags-progressive-delivery.md) | 🟡 | Decoupling deploy from release — flag types & lifespans, targeting & sticky bucketing, canary/blue-green/rolling/dark launch/rings, signal-gated rollouts & auto-rollback, expand/contract migrations, and flag debt. |
...
| [Stream Processing](stream-processing.md) | 🔴 | Computing over unbounded data — batch vs. stream, stateless vs. stateful operators, event time & watermarks, windowing, stream–table duality & joins, exactly-once, checkpointing & scaling, and Lambda vs. Kappa. |
...
| [OpenSpec Across Multiple Repos](openspec-multi-repo.md) | 🟡 | Spec-driven development across a UI / BFF / service split — local vs. cross-cutting specs, the OpenSpec Store & cross-repo changes, contracts as the enforced artifact, coordinating a cross-repo change, the archive process (sync vs. archive + the cross-repo gate), and monorepo vs. polyrepo. |
